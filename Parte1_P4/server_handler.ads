...
package Server_Handler is
   ...
   HASH_SIZE: constant := 10; -- Nota: ¿De qué tamaño?
	type Hash_Range is mod HASH_SIZE;

   function Unbounded_String_Hash(US: ASU.Unbounded_String) return Hash_Range;

   package Clientes_On is new Maps_G_Hash (Key_Type     => ASU.Unbounded_String,
   ...
   package Clientes_Off is new Ordered_Maps_G (Key_Type     => ASU.Unbounded_String,
